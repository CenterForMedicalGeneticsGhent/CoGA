...
from ..schemas import (
    ...
    FamilyImportDatasetPlanOut,
    FamilyImportExistingDataOut,
...
async def _hpo_annotation_count(
    session: AsyncSession,
    *,
    family_uuid: str,
) -> int:
...
def _existing_data_item(
    ...
    *,
    ...
    return FamilyImportExistingDataOut(
        data_type=data_type,
        records=records,
        sample_id=sample_id,
        source=source,
...
def _nonzero_existing_data(
    items: list[FamilyImportExistingDataOut],
) -> list[FamilyImportExistingDataOut]:
    return [item for item in items if item.records > 0]


async def _existing_dataset_data_for_plan(
    session: AsyncSession,
    *,
    summary: FamilyImportDatasetSummary,
    family_context: FamilyMetadataContext | None,
    sample_contexts: dict[str, SampleMetadataContext],
) -> tuple[list[FamilyImportExistingDataOut], list[str]]:
    ...
    if dataset_type == "snv":
        if not family_context.assembly_name:
            return [], ["Small-variant conflict detection needs a single linked assembly."]
        count = await count_family_small_variants(
            family_context.assembly_name,
            family_context.family_uuid,
            project_ids=family_context.project_ids,
        )
        items.append(_existing_data_item("small_variants", records=count))
    elif dataset_type == "haplotypes":
        if family_context.assembly_name:
            small_count = await count_family_small_variants(
                family_context.assembly_name,
                family_context.family_uuid,
                project_ids=family_context.project_ids,
            )
            items.append(_existing_data_item("small_variants", records=small_count, source="glimpse2"))
        else:
            notes.append("GLIMPSE2 small-variant conflict detection needs a single linked assembly.")
        haplotype_count = await count_interval_track_source_rows(
            session,
            family_uuid=family_context.family_uuid,
            track_type="haplotype",
            source="glimpse2",
        )
        items.append(_existing_data_item("haplotype_blocks", records=haplotype_count, source="glimpse2"))
    elif dataset_type == "sv_needlr":
        if not family_context.assembly_name:
            return [], ["Structural-variant conflict detection needs a single linked assembly."]
        count = await count_family_structural_variants(
            family_context.assembly_name,
            family_context.family_uuid,
            project_ids=family_context.project_ids,
            source="needlr",
        )
        items.append(_existing_data_item("structural_variants", records=count, source="needlr"))
    elif dataset_type == "repeats_trgt":
        count = await _repeat_expansion_count(session, sample_contexts=sample_contexts)
        items.append(_existing_data_item("repeat_expansions", records=count, source="trgt"))
    elif dataset_type == "wisecondorx":
        for sample_id in summary.samples:
            sample_context = sample_contexts.get(sample_id)
            if sample_context is None:
                continue
            bins = await _interval_track_count(
                session,
                sample_context=sample_context,
                track_type="coverage",
                source="wisecondorx",
            )
            segments = await _interval_track_count(
                session,
                sample_context=sample_context,
                track_type="segments",
                source="wisecondorx",
            ...
            items.extend(
                [
                    _existing_data_item("coverage", records=bins, sample_id=sample_id, source="wisecondorx"),
                    _existing_data_item("segments", records=segments, sample_id=sample_id, source="wisecondorx"),
                ]
            )
    elif dataset_type == "qdnaseq":
        for sample_id in summary.samples:
            sample_context = sample_contexts.get(sample_id)
            if sample_context is None:
                continue
            ...
            items.extend(
                [
                    _existing_data_item("coverage", records=bins, sample_id=sample_id, source="qdnaseq"),
                    _existing_data_item("segments", records=segments, sample_id=sample_id, source="qdnaseq"),
                ]
            )
    elif dataset_type == "apcad":
        target_samples = summary.samples or list(sample_contexts)
        for sample_id in target_samples:
            sample_context = sample_contexts.get(sample_id)
            if sample_context is None:
                continue
            count = await _interval_track_count(
                session,
                sample_context=sample_context,
                track_type="apcad",
            )
            items.append(_existing_data_item("apcad", records=count, sample_id=sample_id, source="apcad"))
    elif dataset_type == "pcf":
        for sample_id in summary.samples:
            sample_context = sample_contexts.get(sample_id)
            if sample_context is None:
                continue
            count = await count_interval_track_source_rows(
                session,
                sample_uuid=sample_context.sample_uuid,
                track_type=APCAD_PCF_TRACK_TYPE,
                source=APCAD_PCF_SOURCE,
            )
            items.append(_existing_data_item("apcad_pcf", records=count, sample_id=sample_id, source=APCAD_PCF_SOURCE))
    elif dataset_type == "paraphase":
        for sample_id in summary.samples:
            sample_context = sample_contexts.get(sample_id)
            if sample_context is None:
                continue
            count = await _paraphase_count(session, sample_context=sample_context)
            items.append(_existing_data_item("paraphase", records=count, sample_id=sample_id))
    elif dataset_type == "phenotypes":
        count = await _hpo_annotation_count(session, family_uuid=family_context.family_uuid)
        items.append(_existing_data_item("hpo_annotations", records=count))
    return _nonzero_existing_data(items), notes


def _dataset_dependency_notes(dataset_type: str) -> list[str]:
    if dataset_type == "haplotypes":
        return [
            "Haplotype and segregation views may need recalculation after GLIMPSE2 changes.",
        ]
    if dataset_type == "apcad":
        return [
            "APCAD visualization caches and shared-haplotype summaries may need refresh after replacement.",
        ]
    if dataset_type == "phenotypes":
        return [
            "Phenotype-dependent family analyses are marked stale when HPO annotations change.",
        ]
    if dataset_type in {"snv", "sv_needlr", "repeats_trgt"}:
        return [
            "Variant summaries and filters should be refreshed after import completes.",
        ]
    return []


def _register_only_plan(summary: FamilyImportDatasetSummary) -> bool:
    if summary.dataset_type == "haplotypes" and summary.samples and "family_vcf" not in summary.summary:
        return True
    message = summary.message or ""
    return "Registered only" in message or "provenance" in message


def _planned_dataset_action(
    *,
    summary: FamilyImportDatasetSummary,
    existing_data: list[FamilyImportExistingDataOut],
    conflict_mode: str,
) -> tuple[str, str]:
    if not summary.enabled:
        return "skip", summary.message or "Dataset is disabled or not present."
    if summary.status in {"skipped", "disabled"}:
        return "skip", summary.message or "Dataset will not be imported."
    if summary.status not in {"valid", "warning"}:
        return "blocked", summary.message or "Dataset must pass validation before import."
    if _register_only_plan(summary):
        return "register", "Dataset will be registered as package provenance; deep import is not implemented for this file shape."
    if not existing_data:
        return "new", "No existing data of this type was detected."
    if conflict_mode == "cancel":
        return "blocked", "Existing data was detected and the selected policy is cancel."
    if conflict_mode == "overwrite":
        return "replace", "Existing source-specific data will be replaced before the new dataset is imported."
    if summary.dataset_type in {"wisecondorx", "qdnaseq", "apcad", "pcf", "paraphase"}:
        return "merge", "Existing sample-level data will be skipped in update mode; missing sample data will be imported."
    if summary.dataset_type == "phenotypes":
        return "merge", "HPO annotations are upserted, preserving unaffected terms."
    return "skip", "Existing family-level data will be kept and this dataset will be skipped in update mode."


async def build_family_package_import_plan(
    session: AsyncSession,
    *,
    folder_path: str | Path,
    project_id: str | None,
    ...
    normalized_conflict_mode = _normalized_conflict_mode(conflict_mode)
    messages: list[str] = []
    ...
    family_context: FamilyMetadataContext | None = None
    sample_contexts: dict[str, SampleMetadataContext] = {}
    if bundle is not None and validation.family_id:
        existing = await _fetch_existing_family(session, family_id=validation.family_id)
        existing_family = existing is not None
        if existing is not None:
            if normalized_conflict_mode == "cancel":
                errors.append(
                    _issue(
                        "existing_family",
                        f"Family '{validation.family_id}' already exists; choose update or overwrite to import data.",
                    )
                )
            existing_sample_set = {str(sample_id) for sample_id in existing.get("sample_ids", []) if sample_id}
            existing_samples = sorted(existing_sample_set.intersection(bundle.ped.sample_ids))
            if existing_sample_set != set(bundle.ped.sample_ids):
                errors.append(
                    _issue(
                        "existing_family_sample_mismatch",
                        "Existing family has different sample IDs; incremental import is blocked.",
                    )
                )
            else:
                try:
                    family_context = await build_family_metadata_context(
                        session,
                        family_identifier=validation.family_id,
                        user=user,
            ...
            existing_samples = await _existing_sample_ids(session, bundle.ped.sample_ids)
            if existing_samples:
                preview = ", ".join(existing_samples[:10])
                suffix = "" if len(existing_samples) <= 10 else f", and {len(existing_samples) - 10} more"
                errors.append(
    ...
    plan_validation = validation.model_copy(
        update={
    ...
    dataset_plans: list[FamilyImportDatasetPlanOut] = []
    for summary in plan_validation.datasets:
        existing_data, detection_notes = await _existing_dataset_data_for_plan(
            session,
            summary=summary,
            family_context=family_context,
            sample_contexts=sample_contexts,
        )
        action, action_message = _planned_dataset_action(
            summary=summary,
            existing_data=existing_data,
            conflict_mode=normalized_conflict_mode,
        )
        dependencies = [*detection_notes, *_dataset_dependency_notes(summary.dataset_type)]
    ...
    if existing_family:
        messages.append("This is an incremental import into an existing family.")
    else:
        messages.append("This is an initial family import.")
    if any(item.planned_action == "blocked" for item in dataset_plans):
        messages.append("One or more datasets are blocked by validation or conflict policy.")
    return FamilyPackageImportPlanOut(
        valid=plan_validation.valid and not any(item.planned_action == "blocked" for item in dataset_plans),
        family_id=plan_validation.family_id,
        target_mode="incremental" if existing_family else "initial",
        conflict_mode=normalized_conflict_mode,  # type: ignore[arg-type]
