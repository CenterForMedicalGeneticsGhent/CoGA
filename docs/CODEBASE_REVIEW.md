...
| 27 | Repeat-expansion `STATUS_COLORS` + tooltip duplicated across two tracks — **FIXED** (extracted `STATUS_COLORS` + a `RepeatLocusTooltip` component into `repeatExpansionHelpers.tsx`; both tracks import them. The `trackY/trackHeight` geometry was left per-track — it differs between the two) | `frontend/src/components/visualizations/GenomeRepeatExpansionTrack.tsx` (25-30) | `RepeatExpansionTrack` (21-26) and `GenomeRepeatExpansionTrack` (25-30) declare a byte-identical `STATUS_COLORS`; the tooltip JSX (three divs), the `STATUS_COLORS[item.status]?.() || cssVar('--color-repeat-unknown')` lookup, and the `trackY/trackHeight` geometry are all duplicated. | Extract `STATUS_COLORS`, a `RepeatLocusTooltip` component, a `getRepeatColor(status)` helper (and optionally `computeRepeatTrackLayout(height)`) into a shared `repeatExpansionHelpers.ts`; import into both. | S | ~20 lines removed; status-color/tooltip changes happen once. No runtime change. |
